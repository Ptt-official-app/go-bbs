...
func TestOpenFormosaBBSFileHeaderFile(t *testing.T) {
	type args struct {
		filename string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := OpenFormosaBBSFileHeaderFile(tt.args.filename)
			if (err != nil) != tt.wantErr {
				t.Errorf("OpenFormosaBBSFileHeaderFile() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("OpenFormosaBBSFileHeaderFile() = %v, want %v", got, tt.want)
			}
		})
	}
}
