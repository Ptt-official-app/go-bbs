...
func TestAppendFileHeaderFileRecord(t *testing.T) {
	...
	for index, header := range headers {
		CheckPttFileHeader(t, index, header, &expected[index])
		err = os.Remove("testcase/file/" + header.Filename())
		if err != nil {
			t.Error(err)
		}
	}
...
// appending repeated filename
func TestAppendFileHeaderFileRecordWithError(t *testing.T) {
	input := []FileHeader{
		{
			filename:  "M.1599059246.A.CF6",
			modified:  time.Date(2020, time.September, 02, 15, 31, 28, 0, time.UTC),
	...
	AppendFileHeaderFileRecord("testcase/file/77.DIR", &input[0])

	err := AppendFileHeaderFileRecord("testcase/file/77.DIR", &input[1])
	if err == nil {
		t.Errorf("Repeated filename in index %d without error", 1)
	}
	...
	for index, header := range headers {
		CheckPttFileHeader(t, index, header, &input[index])
		err = os.Remove("testcase/file/" + header.Filename())
		if err != nil {
			t.Error(err)
		}
